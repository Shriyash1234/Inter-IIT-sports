import React from 'react'
import './CSS/sports.css'
const Sports = () => {
  return (
    <section className='sports'>
       <h1 className='sports-heading'>Sports</h1>
       <div className='sports-names'>
        <div className='sport-div'>Athletics</div>
        <div className='sport-div'>Badminton</div>
        <div className='sport-div'>Basketball</div>
        <div className='sport-div'>Cricket</div>
        <div className='sport-div'>Football</div>
        <div className='sport-div'>Hockey</div>
        <div className='sport-div'>Lawn Tennis</div>
        <div className='sport-div'>Squash</div>
        <div className='sport-div'>Swimming</div>
        <div className='sport-div'>Table Tennis</div>
        <div className='sport-div'>Volleyball</div>
        <div className='sport-div'>Water Polo</div>
        <div className='sport-div'>Weighlifting</div>
       </div>
    </section>
  )
}

export default Sports
